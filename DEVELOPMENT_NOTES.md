...
- 凍結フォールバック扱いで、UI 的な新機能はブラウザ版だけに入れる。ただし **(a) 音声処理の改善はチェーンの乖離を防ぐため両方に反映する**、**(b) ブラウザでは原理的にできないバッチ / 自動化系の運用補助ツールはシェル側に追加してよい**（例: `podcast_watch.sh`）
- **`podcast_watch.sh`** (2026-05): フォルダ監視のフロントエンド。`podcast_auto.sh` を変えずに wrap し、入力フォルダをポーリングして動画 / 音声ファイルを自動処理する。サイズ安定待ち（コピー中の取りこぼし防止）、`done/` `failed/` 退避、`watch.log`、`--once`（既存分を 1 巡して終了）。新規依存なし。bash 3.2 (`/bin/bash`) で `--once` / 常駐の両モードを検証済み。音声入力は `podcast_auto.sh` が `[0:a]` を使うのでそのまま通る（監視側は拡張子集合に音声を含めるだけ）
- **`install-watch-agent.sh`** (2026-05): `podcast_watch.sh` を launchd の LaunchAgent として常駐登録するインストーラ (macOS)。実パスで plist を生成して `~/Library/LaunchAgents/<label>.plist` に置き `launchctl load`。`RunAtLoad`+`KeepAlive`、`PATH` に `/opt/homebrew/bin` を含める (launchd の最小 PATH 対策、Wraptalk.app と同じ事情)、ログは out-dir の `agent.out/err.log`。`--print` で plist を出力のみ (plutil 検証用 / 手動管理用)、`--uninstall` で除去。plist は `plutil -lint` で検証 (実 load は各自のマシンで)
